...
def perform_dns_query(resolver: dns.resolver.Resolver, query: str, expect_type: str,
                      expect_content: Optional[str]) -> bool:
    try:
        answers = resolver.resolve(query)
        if expect_type == 'A' and answers.rrset.rdtype != dns.rdatatype.A:
            return False
        if expect_type == 'CNAME' and answers.rrset.rdtype != dns.rdatatype.CNAME:
            return False
        if expect_content:
...
def run_stress_test(server: str, port: int, query: str, expect_type: str,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for _ in range(num_requests):
            ...
            futures.append(
                executor.submit(perform_dns_query, resolver, test_query,
                                expect_type, expect_content)
            )
...
def main():
    ...
    parser.add_argument('--expect-type', choices=['A', 'CNAME'], help='Expected record type')
    ...
    result = run_stress_test(
        args.server, args.port, args.query, args.expect_type,
